  ...
  for (let i = 0; i < cantidad; i++) {
    ...
    const estudiante = await repo.save(
      repo.create({
        ...
        tipo_clase: faker.helpers.arrayElement(TIPOS_CLASE),
        demo_seed: demoSeed,
      })
    );
...
async function crearInstructoresDemo(cantidad, sedes, demoSeed, extendidos = false) {
  ...
  for (let i = 0; i < cantidad; i++) {
    ...
    if (extendidos) {
      data.especialidad = ESPECIALIDADES[data.tipo_clase];
...
async function crearResultadosExamen(estudiantes, demoSeed) {
...
async function seedAcademicoDemo() {
  ...
  const resultados = await crearResultadosExamen(estudiantes, demoSeed);
  ...
  return {
    ...
    creados: {
      ...
      ResultadosExamen: resultados.length,
    },
  };
}
