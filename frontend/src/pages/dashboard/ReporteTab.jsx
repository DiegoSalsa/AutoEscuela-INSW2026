...
function porcentajeDisponibilidad(usoFlota) {
  const disponibles = Number(usoFlota?.vehiculosDisponibles || 0);
  const total = Number(usoFlota?.totalFlota || 0);
  return total > 0 ? Number(((disponibles / total) * 100).toFixed(1)) : 0;
}
...
function exportarExcel(reporte) {
  ...
  if (reporte.metricas?.uso_flota) {
    ...
    const disponibilidad = porcentajeDisponibilidad(uf);
    const filas = [
      ['Vehiculos en mantencion', uf.vehiculosOcupados, 'vehiculos'],
              ...
              {METRICAS_DISPONIBLES.map((m) => {
                ...
                return (
                  <button key={m.id} type="button" onClick={() => toggleMetrica(m.id)}
                    className={`flex items-center space-x-2 px-4 py-2.5 rounded-lg border text-sm font-medium transition-all ${selected ? 'bg-primary text-white border-primary shadow-sm' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'
                      }`}>
                    <span>{m.icon}</span>
                    <span>{m.label}</span>
            ...
            {reporte.metricas?.uso_flota && (
              <div className="border border-gray-200 rounded-xl p-5">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-gray-600">En mantencion</span><span className="font-bold" style={{ color: '#FF5722' }}>{reporte.metricas.uso_flota.vehiculosOcupados}</span></div>
                  ...
                  <div className="pt-2 border-t border-gray-100 flex justify-between"><span className="text-gray-600">% Disponibilidad</span><span className="font-bold text-primary">{porcentajeDisponibilidad(reporte.metricas.uso_flota)}%</span></div>
                </div>
              </div>
            )}
