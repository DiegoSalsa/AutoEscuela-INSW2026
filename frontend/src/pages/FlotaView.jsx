...
import { subirImagenVehiculo } from '../service/vehiculos.Service.js';
...
export default function FlotaView({ sedeActiva }) {
  ...
  const handleImagenChange = async (vehiculoId, file) => {
    if (!file) return;
    setUploadingId(vehiculoId);
    setUploadError('');
    try {
      await subirImagenVehiculo(vehiculoId, file);
      await cargarDatos();
    } catch (err) {
      console.error('Error upload imagen vehiculo:', err);
      setUploadError(err.message || 'No se pudo subir la imagen del vehiculo');
    } finally {
      setUploadingId(null);
    }
  };
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {vehiculos.map((v, idx) => {
            ...
            return (
              <div key={v.id || idx} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-all">
                <div className="h-36 bg-gray-100 relative">
                  {v.imagen_url ? (
                    <img
                      src={v.imagen_url}
                      alt={`${v.modelo || 'Vehiculo'} ${v.patente || ''}`}
                      className="w-full h-full object-cover"
                      loading="lazy"
                    />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-gray-300">
                      <svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2"/><circle cx="7" cy="17" r="2"/><path d="M9 17h6"/><circle cx="17" cy="17" r="2"/></svg>
                    </div>
                  )}
                  <label className="absolute right-3 bottom-3 bg-white/95 hover:bg-white text-gray-700 text-xs font-semibold px-3 py-1.5 rounded-lg shadow-sm cursor-pointer">
                    {uploadingId === v.id ? 'Subiendo...' : 'Subir imagen'}
                    <input
                      type="file"
                      accept="image/*"
                      className="hidden"
                      disabled={uploadingId === v.id}
                      onChange={(e) => handleImagenChange(v.id, e.target.files?.[0])}
                    />
                  </label>
                </div>
                <div className="p-5">
